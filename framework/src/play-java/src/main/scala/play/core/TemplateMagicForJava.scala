...
object PlayMagicForJava {
  ...
  implicit def javaFieldtoScalaField(jField: play.data.Form.Field): play.api.data.Field = {
    ...
    new play.api.data.Field(null, jField.name, jField.constraints.asScala.map { jT =>
      jT._1 -> jT._2.asScala
    }, Option(jField.format).map(f => f._1 -> f._2.asScala), jField.errors.asScala.map { jE =>
      play.api.data.FormError(jE.key, jE.messages.asScala, jE.arguments.asScala)
    }, Option(jField.value)) {
  ...
  implicit def implicitJavaMessages: play.api.i18n.Messages =
    try {
      ...
      case NonFatal(_) =>
        play.api.i18n.Messages(play.api.i18n.Lang.defaultLang,
                               play.api.i18n.Messages.messagesApiCache(play.api.Play.current))
    }
}
