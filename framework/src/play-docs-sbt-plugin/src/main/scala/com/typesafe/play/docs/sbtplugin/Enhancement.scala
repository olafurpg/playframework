...
object Enhancement {
  ...
  val enhanceJavaClasses =
    (javaManualSourceDirectories in Test,
     ...
     streams) map { (sourceDirs, deps, analysis, classes, inputs, s) =>
      ...
      val lastEnhanced =
        if (timestampFile.exists) IO.read(timestampFile).toLong
        else Long.MinValue
      val javaClasses =
        (sourceDirs ** "*.java").get flatMap { sourceFile =>
      ...
      val javaClassesWithGeneratedAccessors =
        javaClasses.filter(PropertiesEnhancer.generateAccessors(classpath, _))
      val javaClassesWithAccessorsRewritten =
        javaClasses.filter(PropertiesEnhancer.rewriteAccess(classpath, _))

      if (javaClassesWithGeneratedAccessors.nonEmpty ||
          javaClassesWithAccessorsRewritten.nonEmpty) {
        ...
        existing.foreach {
          case (_, compileSetup) =>
            agg.store.set(updatedAnalysis, compileSetup)
        }
