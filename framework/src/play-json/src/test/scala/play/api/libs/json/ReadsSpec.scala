...
object ReadsSpec extends org.specs2.mutable.Specification {
  ...
  "Local date/time" should {
    ...
    lazy val correctedReads = Reads.localDateTimeReads(DateTimeFormatter.ISO_DATE_TIME, _.drop(1))

    val CustomReads2 =
      Reads.localDateTimeReads(DateTimeFormatter.ofPattern("dd/MM/yyyy, HH:mm:ss"), _.drop(2))

    "be successfully read from number" in {
      reads(JsNumber(BigDecimal valueOf 123L)).aka("read date") must_==
        JsSuccess(LocalDateTime.ofInstant(Instant.ofEpochMilli(123L), ZoneId.systemDefault))
    }
    ...
    "be successfully read with default implicit" >> {
      ...
      "from '2011-12-03T10:15:30+01:00[Europe/Paris]' (with time zone)" in {
        reads(JsString("2011-12-03T10:15:30+01:00[Europe/Paris]")).aka("read date") must_==
  ...
  "Zoned date/time" should {
    ...
    @inline def dateTime(input: String) =
      try {
        ...
        case _: Throwable =>
          LocalDateTime.parse(input, DateTimeFormatter.ISO_DATE_TIME).atZone(ZoneId.systemDefault)
      }

    lazy val correctedReads = Reads.zonedDateTimeReads(DateTimeFormatter.ISO_DATE_TIME, _.drop(1))

    val CustomReads2 =
      Reads.zonedDateTimeReads(DateTimeFormatter.ofPattern("dd/MM/yyyy, HH:mm:ss"), _.drop(2))

    "be successfully read from number" in {
      reads(JsNumber(BigDecimal valueOf 123L)).aka("read date") must_==
        JsSuccess(ZonedDateTime.ofInstant(Instant.ofEpochMilli(123L), ZoneId.systemDefault))
    }
  ...
  "Local date" should {
    ...
    lazy val correctedReads = Reads.localDateReads(DateTimeFormatter.ISO_DATE, _.drop(1))

    val CustomReads2 = Reads.localDateReads(DateTimeFormatter.ofPattern("dd/MM/yyyy"), _.drop(2))

    "be successfully read from number" in {
      val d = LocalDate.now(Clock.fixed(Instant.ofEpochMilli(123L), ZoneId.systemDefault))

      reads(JsNumber(BigDecimal valueOf 123L)).aka("read date") must_== JsSuccess(d)
    }
    ...
    "be successfully read with default implicit from '2011-12-03'" in {
      reads(JsString("2011-12-03")).aka("read date") must_== JsSuccess(date("2011-12-03"))
    }
    ...
    "be successfully read from corrected string" >> {
      ...
      "with custom formatter" in {
        CustomReads2.reads(JsString("# 03/12/2011")).aka("read date") must_== JsSuccess(d)
      }
    }
  }
  ...
  "Instant" should {
    ...
    lazy val correctedReads = Reads.instantReads(DateTimeFormatter.ISO_DATE_TIME, _.drop(1))

    val CustomReads2 =
      Reads.instantReads(DateTimeFormatter.ofPattern("dd/MM/yyyy, HH:mm:ss"), _.drop(2))
    ...
    "be successfully read with default implicit" >> {
      "from '2011-12-03T10:15:30'" in {
        reads(JsString("2011-12-03T10:15:30")).aka("read date") must_==
          JsSuccess(instant("2011-12-03T10:15:30"))
      }
