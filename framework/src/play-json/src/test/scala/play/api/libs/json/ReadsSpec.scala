...
object ReadsSpec extends org.specs2.mutable.Specification {
  ...
  "Local date/time" should {
    ...
    "be successfully read from number" in {
      reads(JsNumber(BigDecimal valueOf 123L)).aka("read date") must_==
        JsSuccess(LocalDateTime.ofInstant(Instant.ofEpochMilli(123L),
    ...
    "not be read from invalid string" in {
      reads(JsString("invalid")) aka "read date" must beLike {
        case JsError((_,
                      ValidationError("error.expected.date.isoformat" :: Nil,
                                      _) :: Nil) :: Nil) =>
          ok
      }
    }
  ...
  "Zoned date/time" should {
    ...
    "be successfully read from number" in {
      reads(JsNumber(BigDecimal valueOf 123L)).aka("read date") must_==
        JsSuccess(ZonedDateTime.ofInstant(Instant.ofEpochMilli(123L),
                                          ZoneId.systemDefault))
    }
  ...
  "Local date" should {
    ...
    "be successfully read from number" in {
      val d = LocalDate.now(
          Clock.fixed(Instant.ofEpochMilli(123L), ZoneId.systemDefault))
  ...
  "Instant" should {
    ...
    lazy val correctedReads =
      Reads.instantReads(DateTimeFormatter.ISO_DATE_TIME, _.drop(1))
