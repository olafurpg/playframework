...
sealed trait PathNode {
  ...
  private[json] def splitChildren(
      json: JsValue): List[Either[(PathNode, JsValue), (PathNode, JsValue)]]
...
case class JsPath(path: List[PathNode] = List()) {
  ...
  def lazyReadNullable[T](r: => Reads[T]): Reads[Option[T]] =
    Reads(js => Reads.nullable[T](this)(r).reads(js))
  ...
  def lazyFormat[T](r: => Reads[T], w: => Writes[T]): OFormat[T] =
    OFormat[T](lazyRead(r), lazyWrite(w))
  ...
  object json {
    ...
    def pickBranch[A <: JsValue](reads: Reads[A]): Reads[JsObject] =
      Reads.jsPickBranch[A](self)(reads)
