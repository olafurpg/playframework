...
object JsMacroImpl {
  ...
  def macroImpl[A, M[_]](c: Context,
    ...
    val applies = companionType.declaration(stringToTermName("apply")) match {
    ...
    val applyMethod = Function(
        params.foldLeft(List[ValDef]())((l, e) =>
              l :+ ValDef(Modifiers(PARAM),
                          newTermName(e.name.encoded),
                          TypeTree(),
                          EmptyTree)),
        Apply(
    ...
    val importFunctionalSyntax = Import(
        functionalSyntaxPkg, List(ImportSelector(nme.WILDCARD, -1, null, -1)))
    if (!hasRec) {
      val block = Block(
                      ...
                      Template(
                          ...
                          List(
                              ...
                              ValDef(
                                  ...
                                  AppliedTypeTree(Ident(matag.tpe.typeSymbol),
                                                  List(TypeTree(atag.tpe))),
                                  finalTree
                              )
                          )
                      )
