...
trait ConstraintReads {
  ...
  def map[A](implicit reads: Reads[A]): Reads[collection.immutable.Map[String, A]] =
    Reads.mapReads[A]
  ...
  def minLength[M](m: Int)(
      implicit reads: Reads[M], p: M => scala.collection.TraversableLike[_, M]) =
  ...
  /**
    ...
    */
  def pattern(regex: => scala.util.matching.Regex, error: String = "error.pattern")(
...
trait PathWrites {
  def at[A](path: JsPath)(implicit wrs: Writes[A]): OWrites[A] = OWrites[A] { a =>
  ...
  def jsPick(path: JsPath): Writes[JsValue] = Writes[JsValue] { obj =>
    path(obj).headOption.getOrElse(JsNull)
  }

  def jsPickBranch(path: JsPath): OWrites[JsValue] = OWrites[JsValue] { obj =>
...
trait ConstraintWrites {
  ...
  def pure[A](fixed: => A)(implicit wrs: Writes[A]): Writes[JsValue] = Writes[JsValue] { js =>
    wrs.writes(fixed)
  }
  ...
  def map[A](implicit writes: Writes[A]): OWrites[collection.immutable.Map[String, A]] =
    Writes.mapWrites[A]
