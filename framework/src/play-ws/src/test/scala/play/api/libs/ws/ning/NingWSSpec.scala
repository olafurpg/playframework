...
object NingWSSpec extends PlaySpecification with Mockito {
  ...
  "Ning WS" should {
    ...
    "support http headers" in new WithApplication {
      val req: client.Request = WS
        .url("http://playframework.com/")
        .withHeaders("key" -> "value1", "key" -> "value2")
        .asInstanceOf[NingWSRequest]
        .buildRequest()
    ...
    "not make Content-Type header if there is Content-Type in headers already" in new WithApplication {
      ...
      val req: client.Request = WS
        ...
        .asInstanceOf[NingWSRequest]
        .buildRequest()
      req.getHeaders.get("Content-Type").asScala must containTheSameElementsAs(
          Seq("fake/contenttype; charset=utf-8"))
    }
    ...
    "Have form params on POST of content type application/x-www-form-urlencoded" in new WithApplication {
      ...
      req.getFormParams.asScala must containTheSameElementsAs(
          List(new com.ning.http.client.Param("param1", "value1")))
    }
    ...
    "Keep the charset if it has been set manually with a charset" in new WithApplication {
      ...
      val req: client.Request = WS
        ...
        .buildRequest()
      req.getHeaders.get("Content-Type").asScala must containTheSameElementsAs(
          Seq("text/plain; charset=US-ASCII"))
    }
    ...
    "not support invalid timeout" in new WithApplication {
      WS.url("http://playframework.com/").withRequestTimeout(-1) should throwAn[
