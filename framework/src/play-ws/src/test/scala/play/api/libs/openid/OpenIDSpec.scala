...
object OpenIDSpec extends Specification with Mockito {
  ...
  "OpenID" should {
    ...
    "generate a valid redirectUrl" in {
      ...
      val openId = new WsOpenIdClient(ws, new WsDiscovery(ws))
      val redirectUrl =
        Await.result(openId.redirectURL("http://example.com", "http://foo.bar.com/returnto"),
                     dur)
    ...
    "verify the response using discovery on the claimed Identifier" in {
      ...
      val responseQueryString =
        openIdResponse - "openid.op_endpoint" + ("openid.op_endpoint" -> Seq(spoofedEndpoint))
    ...
    "fail response verification if direct verification fails" in {
      ...
      ws.response.header(HeaderNames.CONTENT_TYPE) returns Some("application/xrds+xml") thenReturns Some(
          "text/plain")
