...
class DisabledComplainingHostnameVerifier extends HostnameVerifier {
  ...
  override def verify(hostname: String, sslSession: SSLSession): Boolean = {
    ...
    if (!hostNameMatches) {
      val msg =
        s"""Hostname verification failed on hostname $hostname, but the connection was accepted because ws.ssl.disableHostnameVerification is enabled.  Please fix the X.509 certificate on the host to remove this warning."""
      logger.warn(msg)
    }
    true
  }
