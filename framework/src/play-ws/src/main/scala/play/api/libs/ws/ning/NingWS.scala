...
case class NingWSRequest(client: NingWSClient,
  ...
  def withRequestTimeout(timeout: Long): WSRequest = {
    require(timeout >= 0 && timeout <= Int.MaxValue,
            s"Request timeout must be between 0 and ${Int.MaxValue}")
  ...
  private[libs] def auth(username: String,
                         password: String,
                         scheme: AuthScheme = AuthScheme.BASIC): Realm = {
    (new RealmBuilder)
      .setScheme(scheme)
...
@Singleton
class NingWSAPI @Inject()(environment: Environment,
                          clientConfig: NingWSClientConfig,
                          lifecycle: ApplicationLifecycle) extends WSAPI {
...
case class NingWSResponse(ahcResponse: AHCResponse) extends WSResponse {
  ...
  lazy val allHeaders: Map[String, Seq[String]] = {
    TreeMap[String, Seq[String]]()(CaseInsensitiveOrdered) ++ mapAsScalaMapConverter(
        ahcResponse.getHeaders).asScala.mapValues(_.asScala)
  }
