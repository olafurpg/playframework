...
abstract class WithApplicationLoader(
    ...
    context: ApplicationLoader.Context = ApplicationLoader.createContext(
          new Environment(new java.io.File("."),
                          ...
                          Mode.Test))) extends Around with Scope {
...
/**
  ...
  */
abstract class WithApplication(val app: Application = FakeApplication())
...
/**
  ...
  */
abstract class WithServer(val app: Application = FakeApplication(),
...
abstract class WithBrowser[WEBDRIVER <: WebDriver](
  ...
  def this(webDriver: Class[WEBDRIVER], app: Application, port: Int) = this(
      WebDriverFactory(webDriver), app, port)
