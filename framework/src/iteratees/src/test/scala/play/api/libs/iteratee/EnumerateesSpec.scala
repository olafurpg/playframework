...
object EnumerateesSpec
  ...
  "Enumeratee.zip" should {
    ...
    "combine the final results into a pair" in {
      Await.result(Enumeratee.zip(Done[Int, Int](2), Done[Int, Int](3)).unflatten, Duration.Inf) must equalTo(
          Step.Done((2, 3), Input.Empty))
    }
  }
  ...
  "Enumeratee.zipWith" should {
    ...
    "combine the final results" in {
      mustExecute(1) { zipEC =>
        Await.result(
            Enumeratee.zipWith(Done[Int, Int](2), Done[Int, Int](3))(_ * _)(zipEC).unflatten,
  ...
  "Enumeratee.mapConcatInput" should {
    ...
    "transform each input element into a sequence of inputs" in {
      mustExecute(2) { mapEC =>
        mustTransformTo(1, 2)(1, 1, 2, 2)(Enumeratee
              .mapConcatInput[Int](x => List(Input.El(x), Input.Empty, Input.El(x)))(mapEC))
      }
    }
  }
    ...
    "transform each input element into the output of an enumerator" in {
      mustExecute(2) { mapFlattenEC =>
        mustTransformTo(1, 2)(1, 1, 2, 2)(
            Enumeratee.mapFlatten[Int](x => Enumerator(x, x))(mapFlattenEC))
      }
    }
  }
    ...
    "transform each input" in {
      mustExecute(2) { mapEC =>
        mustTransformTo(1, 2)(2, 4)(
            Enumeratee.mapInputM[Int]((i: Input[Int]) => Future.successful(i.map(_ * 2)))(mapEC))
      }
    }
  }
  ...
  "Enumeratee.mapM" should {
    ...
    "transform each input element" in {
      mustExecute(2) { mapEC =>
        mustTransformTo(1, 2)(2, 4)(
            Enumeratee.mapM[Int]((x: Int) => Future.successful(x * 2))(mapEC))
      }
    }
  }
  ...
  "Enumeratee.dropWhile" should {
    ...
    "ignore chunks while predicate is valid" in {
      mustExecute(4) { dropWhileEC =>
        val drop3AndConsume =
          Enumeratee.dropWhile[String](_ != "4")(dropWhileEC) &>> Iteratee.consume[String]()
        val enumerator = Enumerator(Range(1, 20).map(_.toString):_*)
        Await.result(enumerator |>>> drop3AndConsume, Duration.Inf) must equalTo(
            Range(4, 20).map(_.toString).mkString)
      }
  ...
  "Enumeratee.take" should {
    ...
    "not execute callback on take 0" in {
      mustExecute(0, 0) { (generateEC, foldEC) =>
        ...
        Await.result(
            enumerator &> Enumeratee.take(0) |>>> Iteratee.fold(0)((_: Int) + (_: Int))(foldEC),
  ...
  "Enumeratee.takeWhile" should {
    ...
    "pass chunks until condition is not met" in {
      mustExecute(4) { takeWhileEC =>
        val take3AndConsume =
          Enumeratee.takeWhile[String](_ != "4")(takeWhileEC) &>> Iteratee.consume()
        val enumerator = Enumerator(Range(1, 20).map(_.toString):_*)
        Await.result(enumerator |>>> take3AndConsume, Duration.Inf) must equalTo(
            List(1, 2, 3).map(_.toString).mkString)
      }
  ...
  "Enumeratee.onIterateeDone" should {
    ...
    "call the callback when the iteratee is done" in {
      mustExecute(1) { doneEC =>
        ...
        mustTransformTo(1, 2, 3)(1, 2, 3)(
            Enumeratee.onIterateeDone(() => count.incrementAndGet())(doneEC))
  ...
  "Enumeratee.map" should {
    ...
    "add one to each of the ints enumerated" in {
      mustExecute(4) { mapEC =>
        val add1AndConsume =
          Enumeratee.map[Int](i => List(i + 1))(mapEC) &>> Iteratee.consume[List[Int]]()
    ...
    "infer its types correctly from previous enumeratee" in {
      mustExecute(0, 0) { (map1EC, map2EC) =>
        val add1AndConsume =
          Enumeratee.map[Int](i => i + 1)(map1EC) ><> Enumeratee.map[Int](i => List(i))(map2EC) &>> Iteratee
            .consume[List[Int]]()
  ...
  "Enumeratee.filter" should {
    ...
    "only enumerate input that satisfies the predicate" in {
      mustExecute(6) { filterEC =>
        mustTransformTo("One", "Two", "Three", "Four", "Five", "Six")("One", "Two", "Six")(
            Enumeratee.filter[String](_.length < 4)(filterEC))
      }
    }
  }
  ...
  "Enumeratee.filterNot" should {
    ...
    "only enumerate input that doesn't satisfy the predicate" in {
      mustExecute(6) { filterEC =>
        mustTransformTo("One", "Two", "Three", "Four", "Five", "Six")("Three", "Four", "Five")(
            Enumeratee.filterNot[String](_.length < 4)(filterEC))
      }
    }
  }
    ...
    "ignores input that doesn't satisfy the predicate and transform the input when matches" in {
      mustExecute(6) { collectEC =>
        mustTransformTo("One", "Two", "Three", "Four", "Five", "Six")("ONE", "TWO", "SIX")(
          ...
          case e@("One" | "Two" | "Six") => e.toUpperCase
        }(collectEC))
      }
    }
  }
      ...
      mustExecute(9, 7, 3) { (mapInputEC, foldEC, mapEC) =>
        val folderIteratee =
          Enumeratee.mapInput[String] {
            ...
            case other => other
          }(mapInputEC) &>> Iteratee.fold[String, String]("")((s, e) => s + e)(foldEC)

        val result =
          Enumerator("He", "ll", "o", "Concat", "Wo", "r", "ld", "Concat", "!") &> Enumeratee
            .grouped(folderIteratee) ><> Enumeratee.map[String](List(_))(mapEC) |>>> Iteratee
            .consume[List[String]]()
  ...
  "Enumeratee.scanLeft" should {
    ...
    "transform elements using a sate" in {
      mustExecute(4) { mapEC =>
        val result =
          Enumerator(1, 2, 3, 4) &> Enumeratee.scanLeft[Int](0)(_ + _) ><> Enumeratee
            .map[Int](List(_))(mapEC) |>>> Iteratee.consume[List[Int]]()
  ...
  "Enumeratee.recover" should {
    ...
    "perform computations and log errors" in {
      mustExecute(3, 3) { (recoverEC, mapEC) =>
        ...
        val result =
          Enumerator(0, 2, 4) &> Enumeratee.recover[Int] { (_, input) =>
