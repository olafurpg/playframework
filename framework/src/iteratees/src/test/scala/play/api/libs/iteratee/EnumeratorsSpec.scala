...
object EnumeratorsSpec
  ...
  "Enumerator.enumerate " should {
    "generate an Enumerator from a singleton Iterator" in {
      mustExecute(1) { foldEC =>
        ...
        val futureOfResult =
          Enumerator.enumerate(iterator) |>>> Enumeratee.take(1) &>> Iteratee.fold(
              List.empty[Int])((r, e: Int) => e :: r)(foldEC)
    ...
    "take as much element as in the iterator in the right order" in {
      mustExecute(50) { foldEC =>
        ...
        val futureOfResult =
          Enumerator.enumerate(iterator) |>>> Enumeratee.take(100) &>> Iteratee.fold(
              Seq.empty[Int])((r, e: Int) => r :+ e)(foldEC)
        val result = Await.result(futureOfResult, Duration.Inf)
    ...
    "work with Seq too" in {
      mustExecute(6) { foldEC =>
        ...
        val futureOfResult =
          Enumerator.enumerate(seq) |>>> Enumeratee.take(100) &>> Iteratee.fold(Seq.empty[Int])((r,
              e: Int) => r :+ e)(foldEC)
        val result = Await.result(futureOfResult, Duration.Inf)
  ...
  "Enumerator.generateM" should {
    "generate a stream of values until the expression is None" in {
      mustExecute(12, 11) { (generateEC, foldEC) =>
        ...
        val enumerator =
          Enumerator.generateM(Future(if (it.hasNext) Some(it.next()) else None))(generateEC)

        Await.result(enumerator |>>> Iteratee.fold[Int, String]("")(_ + _)(foldEC), Duration.Inf) must equalTo(
            "012345678910")
      }
    }
  ...
  "Enumerator.callback1" should {
    ...
    "Call onError on future failure" in {
      val it1 = Iteratee.fold1[String, String](Future.successful(""))((_,
          _) => Future.failed(new RuntimeException()))
      val it2 = Iteratee.fold1[String, String](Future.failed(new RuntimeException()))((_,
          _) => Future.failed(new RuntimeException()))
      val errorCount = new AtomicInteger(0)
    ...
    "close the stream" in {
      ...
      "when done normally" in {
        ...
        mustExecute(2) { fromStreamEC =>
          Await.result(Enumerator.fromStream(stream)(fromStreamEC)(Iteratee.ignore),
                       Duration.Inf)
  ...
  "Enumerator.unfoldM" should {
    "Can be composed with another enumerator (doesn't send EOF)" in {
      mustExecute(12, 12) { (foldEC, unfoldEC) =>
        val enumerator =
          Enumerator.unfoldM[Int, Int](0)(s => Future(if (s > 10) None else Some((s + 1, s + 1))))(
  ...
  "Enumerator.unfold" should {
    "unfolds a value into input for an enumerator" in {
      mustExecute(5) { unfoldEC =>
        val enumerator =
          Enumerator.unfold[Int, Int](0)(s => if (s > 3) None else Some((s + 1, s)))(unfoldEC)
  ...
  "Enumerator.repeatM" should {
    "supply input from a by-name arg" in {
      mustExecute(3) { repeatEC =>
        val count = new AtomicInteger(0)
        val fut =
          Enumerator.repeatM(Future.successful(count.incrementAndGet()))(repeatEC) |>>>
          (Enumeratee.take(3) &>> Iteratee.getChunks[Int])
        Await.result(fut, Duration.Inf) must equalTo(List(1, 2, 3))
      }
    }
  ...
  "Enumerator.outputStream" should {
    "produce the same value written in the OutputStream" in {
      mustExecute(1, 2) { (outputEC, foldEC) =>
        ...
        val promise =
          (enumerator |>>> Iteratee.fold[Array[Byte], Array[Byte]](Array[Byte]())(_ ++ _)(foldEC))
