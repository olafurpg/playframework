...
object IterateesSpec extends Specification with IterateeSpecification with ExecutionSpecification {
  ...
  "Done iteratees" should {
    ...
    "fold input with fold1" in {
      mustExecute(1) { foldEC =>
        mustTranslate3To(5)(it =>
              Iteratee.flatten(it.fold1((a, i) => Future.successful(Done(a + 2, i)),
                                        _ => ???,
                                        (_, _) => ???)(foldEC)))
      }
    }
    ...
    "fold input with flatFold" in {
      mustExecute(1) { foldEC =>
        mustTranslate3To(9)(_.flatFold((_, _) => Future.successful(Done[Int, Int](9)),
                                       _ => ???,
                                       (_, _) => ???)(foldEC))
      }
    }
    ...
    "flatMap directly to result when no remaining input" in {
      mustExecute(1) { flatMapEC =>
        await(Done(3).flatMap((x: Int) => Done[Int, Int](x * 2))(flatMapEC).unflatten) must equalTo(
            Step.Done(6, Input.Empty))
      }
    }
    ...
    "flatMap result and process remaining input with Cont" in {
      mustExecute(1) { flatMapEC =>
        await(Done(3, Input.El("remaining"))
              .flatMap((x: Int) => Cont(in => Done[String, Int](x * 2, in)))(flatMapEC)
              .unflatten) must equalTo(Step.Done(6, Input.El("remaining")))
      }
    }
    ...
    "flatMap result with flatMapM" in {
      mustExecute(1) { flatMapEC =>
        mustTranslate3To(6)(
            _.flatMapM((x: Int) => Future.successful(Done[Int, Int](x * 2)))(flatMapEC))
      }
    }
    ...
    "concatenate unused input with flatMapTraversable" in {
      mustExecute(1) { flatMapEC =>
        await(Done(3, Input.El(List(1, 2)))
  ...
  "Cont iteratees" should {
    ...
    "not overflow the stack when called recursively" in {
      ...
      val flatMapped: Iteratee[Unit, Unit] =
        (0 until overflowDepth).foldLeft[Iteratee[Unit, Unit]](Cont(_ => unitDone)) {
  ...
  "Error iteratees" should {
    ...
    "flatMap to an error" in {
      mustExecute(0) { flatMapEC =>
        await(Error("msg", Input.El("bad")).flatMap((x: Int) => Done("done"))(flatMapEC).unflatten) must equalTo(
            Step.Error("msg", Input.El("bad")))
      }
    }
  }
  ...
  "Iteratee.fold" should {
    ...
    "fold input" in {
      mustExecute(4) { foldEC =>
        await(Enumerator(1, 2, 3, 4) |>>> Iteratee.fold[Int, Int](0)(_ + _)(foldEC)) must equalTo(
            10)
      }
    }
  }
  ...
  "Iteratee.fold2" should {
    ...
    "fold input" in {
      mustExecute(4) { foldEC =>
        ...
        await(Enumerator(1, 2, 3, 4) |>>> Iteratee.fold2[Int, Int](0)(folder)(foldEC)) must equalTo(
            10)
      }
    }
  ...
  "Iteratee.foldM" should {
    ...
    "fold input" in {
      mustExecute(4) { foldEC =>
        await(Enumerator(1, 2, 3, 4) |>>> Iteratee.fold1[Int, Int](Future.successful(0))((x,
                y) => Future.successful(x + y))(foldEC)) must equalTo(10)
      }
    }
  }
