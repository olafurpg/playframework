...
object CharEncoding {
        ...
        code(initial, true).fold({ result =>
          ...
          case (string, remaining) =>
            val newIt = Iteratee.flatten(
                it.feed(Input.El(string)).flatMap(_.feed(in))(defaultExecutionContext))
            Done(newIt)
        })
    }
  ...
  def decode(charset: Charset): Enumeratee[Array[Byte], String] = new Coder[Array[Byte], String] {
    ...
    protected def code(bytes: Array[Byte], last: Boolean) = {
      ...
      val charBuffer =
        CharBuffer.allocate(2 max math.ceil(bytes.length * decoder.averageCharsPerByte).toInt)
  ...
  def encode(charset: Charset): Enumeratee[String, Array[Byte]] = new Coder[String, Array[Byte]] {
    ...
    protected def code(chars: String, last: Boolean) = {
      ...
      val byteBuffer =
        ByteBuffer.allocate(6 max math.ceil(chars.length * encoder.averageBytesPerChar).toInt)
      ...
      if (result.isUnmappable || last && result.isMalformed) {
        ...
        val bytesWithoutBom =
          if (charset.name.startsWith("UTF-") && bytes.length >= 2 && bytes(0) == 0xfe.toByte &&
              bytes(1) == 0xff.toByte) {
            bytes.drop(2)
          } else {
            bytes
