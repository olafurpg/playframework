...
object Traversable {
  ...
  def take[M](count: Int)(
      implicit p: M => scala.collection.TraversableLike[_, M]): Enumeratee[M, M] =
    new Enumeratee[M, M] {
  ...
  /**
    ...
    */
  def splitOnceAt[M, E](
      p: E => Boolean)(implicit traversableLike: M => scala.collection.TraversableLike[E, M],
        ...
        Iteratee.flatten(Future(e.span(p))(pec).map {
          case (prefix, suffix) if suffix.isEmpty =>
            new CheckDone[M, M] { def continue[A](k: K[M, A]) = Cont(step(k)) } &> k(
                Input.El(prefix))
          case (prefix, suffix) =>
            Done(if (prefix.isEmpty) Cont(k) else k(Input.El(prefix)), Input.El(suffix.drop(1)))
        }(dec))
