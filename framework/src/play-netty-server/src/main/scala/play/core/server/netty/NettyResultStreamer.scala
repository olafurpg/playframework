...
object NettyResultStreamer {
  ...
  /**
    ...
    */
  def sendResult(requestHeader: RequestHeader,
                 result: Result,
        ...
        case Right((streaming, connectionHeader)) =>
          ...
          streaming match {
            ...
            case ServerResultUtils.StreamWithStrictBody(body) =>
              ...
              val buffer =
                if (body.isEmpty) ChannelBuffers.EMPTY_BUFFER
                else ChannelBuffers.wrappedBuffer(body)
  ...
  private def nettyStreamIteratee(nettyResponse: HttpResponse,
                                  startSequence: Int,
    ...
    def step(subsequence: Int)(in: Input[Array[Byte]]): Iteratee[Array[Byte], ChannelStatus] =
      in match {
        case Input.El(x) =>
          ...
          nextWhenComplete(sendDownstream(subsequence, false, b),
                           step(subsequence + 1),
    ...
    nextWhenComplete(sendDownstream(startSequence, false, nettyResponse),
                     ...
                     new ChannelStatus(closeConnection, startSequence))
  }

  def createNettyResponse(header: ResponseHeader,
                          connectionHeader: ServerResultUtils.ConnectionHeader,
                          httpVersion: HttpVersion) = {
    val responseStatus = header.reasonPhrase.fold {
      HttpResponseStatus.valueOf(header.status)
    } { phrase =>
      new HttpResponseStatus(header.status, phrase)
    }
  ...
  def nextWhenComplete[E, A](future: ChannelFuture, step: (Input[E]) => Iteratee[E, A], done: A)(
    ...
    if (!ctx.getChannel.isConnected) {
      ...
      Iteratee.flatten(
          future.toScala
            .map[Iteratee[E, A]] { _ =>
              if (ctx.getChannel.isConnected()) Cont(step) else Done(done)
            }
