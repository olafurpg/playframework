...
trait ScalaResultsHandlingSpec
  ...
  "scala body handling" should {
    ...
    def tryRequest[T](result: Result)(block: Try[WSResponse] => T) =
      withServer(result) { implicit port =>
    ...
    "add Content-Length when enumerator contains a single item" in makeRequest(
        Results.Ok("Hello world")) { response =>
