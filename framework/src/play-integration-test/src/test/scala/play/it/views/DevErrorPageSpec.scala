...
object DevErrorPageSpec extends PlaySpecification {
  ...
  "devError.scala.html" should {
    ...
    val testExceptionSource =
      new play.api.PlayException.ExceptionSource("test", "making sure the link shows up") {
    ...
    "show prod error page in prod mode" in {
      val errorHandler =
        new DefaultHttpErrorHandler(Environment.simple(mode = Mode.Prod), Configuration.empty)
