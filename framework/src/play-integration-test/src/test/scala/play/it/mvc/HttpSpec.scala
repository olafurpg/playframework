...
object HttpSpec extends org.specs2.mutable.Specification {
  ...
  "Absolute URL" should {
    ...
    "have HTTP scheme" in {
      (Call("GET", "/playframework").absoluteURL()(req).aka("absolute URL 1") must_== "http://playframework.com/playframework")
        .and(Call("GET", "/playframework").absoluteURL(false)(req).aka("absolute URL 2") must_==
          ("http://playframework.com/playframework"))
    }

    "have HTTPS scheme" in {
      (Call("GET", "/playframework").absoluteURL()(req.copy(secure = true)).aka("absolute URL 1") must_==
          ("https://playframework.com/playframework")) and
      (Call("GET", "/playframework").absoluteURL(true)(req).aka("absolute URL 2") must_==
  ...
  "Web socket URL" should {
    ...
    "have WS scheme" in {
      (Call("GET", "/playframework").webSocketURL()(req).aka("absolute URL 1") must_== "ws://playframework.com/playframework")
        .and(Call("GET", "/playframework").webSocketURL(false)(req).aka("absolute URL 2") must_==
          ("ws://playframework.com/playframework"))
    }

    "have WSS scheme" in {
      (Call("GET", "/playframework").webSocketURL()(req.copy(secure = true)).aka("absolute URL 1") must_==
          ("wss://playframework.com/playframework")) and
      (Call("GET", "/playframework").webSocketURL(true)(req).aka("absolute URL 2") must_==
  ...
  "RequestHeader" should {
    ...
    "parse quoted and unquoted charset" in {
      FakeRequest().withHeaders(HeaderNames.CONTENT_TYPE -> "text/xml; charset=utf-8").charset aka "request charset" must beSome(
          "utf-8")
    }
  }
}
