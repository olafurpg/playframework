...
object ScalaResultsSpec extends PlaySpecification {
  ...
  "support a custom session domain" in {
    "set session on right domain" in withFooDomain {
      Cookies.decode(Ok.withSession("user" -> "alice").header.headers("Set-Cookie")).head.domain must beSome(
          ".foo.com")
    }

    "discard session on right domain" in withFooDomain {
      Cookies.decode(Ok.withNewSession.header.headers("Set-Cookie")).head.domain must beSome(
          ".foo.com")
    }
  }
