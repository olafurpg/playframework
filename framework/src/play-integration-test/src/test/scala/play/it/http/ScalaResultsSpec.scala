...
object ScalaResultsSpec extends PlaySpecification {
  ...
  "support session helper" in withApplication() {
    ...
    val Result(ResponseHeader(_, headers, _), _, _) = Ok("hello")
      .as("text/html")
  ...
  "support a custom application context" in {
    "set session on right path" in withFooPath {
      Cookies
        .decode(Ok.withSession("user" -> "alice").header.headers("Set-Cookie"))
        .head
        .path must_== "/foo"
    }
    ...
    "set flash on right path" in withFooPath {
      Cookies
        .decode(Ok.flashing("user" -> "alice").header.headers("Set-Cookie"))
        .head
        .path must_== "/foo"
    }
  ...
  "support a custom session domain" in {
    ...
    "discard session on right domain" in withFooDomain {
      Cookies
        .decode(Ok.withNewSession.header.headers("Set-Cookie"))
        .head
        .domain must beSome(".foo.com")
    }
  }
  ...
  def withApplication[T](config: (String, Any)*)(block: => T): T =
    running(
        FakeApplication(additionalConfiguration = Map(config:_*) +
              ("play.crypto.secret" -> "foo"))
    )(block)

  def withFooPath[T](block: => T) =
    withApplication("application.context" -> "/foo")(block)

  def withFooDomain[T](block: => T) =
    withApplication("session.domain" -> ".foo.com")(block)
