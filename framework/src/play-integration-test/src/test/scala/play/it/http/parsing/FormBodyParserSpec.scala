...
class FormBodyParserSpec extends PlaySpecification {
  ...
  "The form body parser" should {
    ...
    def parse[A, B](
        body: B, bodyParser: BodyParser[A])(implicit W: Writeable[B]): Either[Result, A] = {
      await(Enumerator(W.transform(body)) |>>> bodyParser(
              FakeRequest().withHeaders(W.contentType.map(CONTENT_TYPE -> _).toSeq:_*)))
    }
    ...
    "bind JSON requests" in new WithApplication {
      parse(Json.obj("name" -> "Alice", "age" -> 42), BodyParsers.parse.form(userForm)) must beRight(
          User("Alice", 42))
    }

    "bind form-urlencoded requests" in new WithApplication() {
      parse(Map("name" -> Seq("Alice"), "age" -> Seq("42")), BodyParsers.parse.form(userForm)) must beRight(
          User("Alice", 42))
    }
    ...
    "allow users to override the error reporting behaviour" in new WithApplication() {
      ...
      parse(Json.obj("age" -> "Alice"),
            BodyParsers.parse.form(userForm,
