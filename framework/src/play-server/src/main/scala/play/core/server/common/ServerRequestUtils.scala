...
object ServerRequestUtils {
  ...
  /**
    ...
    */
  def findSecureProtocol(forwardedHeaderHandler: ForwardedHeaderHandler,
                         headers: Headers,
                         connectionSecureProtocol: Boolean): Boolean = {
    val forwardedSecureFlag: Option[Boolean] =
      forwardedHeaderHandler.remoteProtocol(headers).map(_ == "https")
