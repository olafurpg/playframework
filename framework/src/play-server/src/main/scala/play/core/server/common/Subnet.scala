...
private[common] case class Subnet(ip: InetAddress, cidr: Option[Int] = None) {
  ...
  def isInRange(otherIp: InetAddress) = {
    ...
    ip.getClass == otherIp.getClass && ip.getAddress
      .take(mask / 8)
      .toList
      .equals(otherIp.getAddress.take(mask / 8).toList) && (for {
...
private[common] object Subnet {
  def apply(s: String): Subnet = s.split("/") match {
    case Array(ip, subnet) =>
      Subnet(InetAddress.getByName(ip), Some(subnet.toInt))
    case Array(ip) => Subnet(InetAddress.getByName(ip))
    case _ =>
      throw new IllegalArgumentException(s"$s contains more than one '/'.")
  }
