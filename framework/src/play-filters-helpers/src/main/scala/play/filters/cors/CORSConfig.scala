...
/**
  ...
  *
  */
case class CORSConfig(anyOriginAllowed: Boolean = true,
...
object CORSConfig {
  ...
  /**
    *
    */
  val denyAll: CORSConfig = CORSConfig(anyOriginAllowed = false,
  ...
  private[cors] def fromUnprefixedConfiguration(
    ...
    CORSConfig(
        ...
        isHttpMethodAllowed = config
            .getOptional[Seq[String]]("allowedHttpMethods")
            .map { methods =>
              val s = methods.toSet
              s.contains _
            }
        ...
        isHttpHeaderAllowed = config
            .getOptional[Seq[String]]("allowedHttpHeaders")
            .map { headers =>
