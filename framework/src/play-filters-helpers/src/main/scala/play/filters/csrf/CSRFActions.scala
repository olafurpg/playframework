...
class CSRFAction(next: EssentialAction,
  ...
  def apply(request: RequestHeader) = {
    ...
    if (config.checkMethod(request.method) && config.checkContentType(request.contentType)) {
        ...
        getTokenFromHeader(request, config).map { headerToken =>
          ...
          getTokenFromQueryString(request, config).map { queryStringToken =>
            if (tokenProvider.compareTokens(headerToken, queryStringToken)) {
              ...
              filterLogger
                .trace("[CSRF] Check failed because invalid token found in query string: " +
                  queryStringToken)
      ...
      next(requestWithNewToken)
        .map(result => CSRFAction.addTokenToResponse(config, newToken, request, result))
    } else {
  ...
  private def checkMultipartBody =
    checkBody[MultipartFormData[Unit]](multipartFormData[Unit]({
      case _ =>
        Iteratee.ignore[Array[Byte]].map(_ => MultipartFormData.FilePart("", "", None, ()))
    }), _.dataParts) _

  private def checkBody[T](parser: BodyParser[T], extractor: (T => Map[String, Seq[String]]))(
  ...
  private[csrf] def checkCsrfBypass(request: RequestHeader, config: CSRFConfig): Boolean = {
    if (config.headerBypass) {
      if (request.headers.get(config.headerName).exists(_ == CSRFConfig.HeaderNoCheck)) {
        ...
        filterLogger.trace("[CSRF] Bypassing check because " + config.headerName + ": " +
        ...
        .fold(
            config.cookieName.flatMap { cookie =>
              request.cookies.get(cookie).map { token =>
                result.discardingCookies(DiscardingCookie(cookie,
                                                          domain = Session.domain,
                                                          path = Session.path,
                                                          secure = config.secureCookie))
              }
        ...
        CSRFAction
          ...
          .flatMap { headerToken =>
            ...
            CSRFAction
              ...
              .collect {
                case queryToken if tokenProvider.compareTokens(queryToken, headerToken) =>
                  wrapped(request)
              }
          }
          .getOrElse {
...
object CSRFAddToken {
  ...
  private class CSRFAddTokenAction[A](
      config: CSRFConfig, tokenProvider: TokenProvider, wrapped: Action[A]) extends Action[A] {
    def parser = wrapped.parser
    def apply(request: Request[A]) = {
