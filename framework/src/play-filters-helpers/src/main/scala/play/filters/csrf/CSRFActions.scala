...
class CSRFAction(next: EssentialAction,
  ...
  private def checkFailed(
      req: RequestHeader, msg: String): Iteratee[Array[Byte], Result] =
    Iteratee.flatten(
        clearTokenIfInvalid(req, config, errorHandler, msg) map (Done(_)))
  ...
  def apply(request: RequestHeader) = {
    ...
    if (config.checkMethod(request.method) &&
          ...
          getTokenFromQueryString(request, config).map { queryStringToken =>
            ...
            request.contentType match {
              ...
              case Some("multipart/form-data") =>
                checkMultipartBody(
                    request, headerToken, config.tokenName, next)
    ...
    } else if (getTokenFromHeader(request, config).isEmpty &&
      ...
      val requestWithNewToken =
        request.copy(tags = request.tags + (Token.RequestTag -> newToken))
  ...
  private def checkFormBody =
    checkBody[Map[String, Seq[String]]](tolerantFormUrlEncoded, identity) _
  private def checkMultipartBody =
    checkBody[MultipartFormData[Unit]](multipartFormData[Unit]({
      case _ =>
        Iteratee
          .ignore[Array[Byte]]
          .map(_ => MultipartFormData.FilePart("", "", None, ()))
    }), _.dataParts) _

  private def checkBody[T](
      parser: BodyParser[T], extractor: (T => Map[String, Seq[String]]))(
    ...
    val firstPartOfBody: Iteratee[Array[Byte], Array[Byte]] =
      Traversable.take[Array[Byte]](config.postBodyBuffer.asInstanceOf[Int]) &>> Iteratee
        .consume[Array[Byte]]()
      ...
      Iteratee.flatten(parsedBody.map { parseResult =>
        val validToken = parseResult.fold(
            ...
            body =>
              ...
              } yield tokenProvider.compareTokens(token, tokenFromHeader))
                .getOrElse(false)
        )
...
object CSRFAction {
  ...
  /**
    ...
    */
  private[csrf] def getTokenFromHeader(
      request: RequestHeader, config: CSRFConfig) = {
    val cookieToken = config.cookieName
  ...
  private[csrf] def getTokenFromQueryString(
      request: RequestHeader, config: CSRFConfig) = {
  ...
  private[csrf] def checkCsrfBypass(
    ...
    if (config.headerBypass) {
      if (request.headers
        ...
        filterLogger
          .trace("[CSRF] Bypassing check because " + config.headerName + ": " +
            CSRFConfig.HeaderNoCheck + " header found")
        true
        ...
        filterLogger.trace(
            "[CSRF] Bypassing check because X-Requested-With header found")
        true
      } else {
        false
      }
  ...
  private[csrf] def addTokenToResponse(config: CSRFConfig,
    ...
    if (isCached(result)) {
      ...
      config.cookieName.map {
        // cookie
        name =>
          result.withCookies(Cookie(name,
                                    newToken,
                                    path = Session.path,
...
object CSRFCheck {
  ...
  private class CSRFCheckAction[A](config: CSRFConfig,
    ...
    def apply(request: Request[A]) = {
      ...
      if (CSRFAction.checkCsrfBypass(request, config) ||
        ...
        CSRFAction
          .getTokenFromHeader(request, config)
          .flatMap { headerToken =>
            // Get token from query string
            CSRFAction
              .getTokenFromQueryString(request, config)
              ...
              .collect {
                case queryToken
                    if tokenProvider.compareTokens(queryToken, headerToken) =>
                  wrapped(request)
              }
          }
          .getOrElse {
  ...
  def apply[A](action: Action[A],
               errorHandler: ErrorHandler = CSRF.DefaultErrorHandler,
               config: CSRFConfig = CSRFConfig.global): Action[A] =
    new CSRFCheckAction(
        config, new TokenProviderProvider(config).get, errorHandler, action)
}
...
object CSRFAddToken {
  ...
  /**
    ...
    */
  def apply[A](
      action: Action[A], config: CSRFConfig = CSRFConfig.global): Action[A] =
    new CSRFAddTokenAction(
        config, new TokenProviderProvider(config).get, action)
}
