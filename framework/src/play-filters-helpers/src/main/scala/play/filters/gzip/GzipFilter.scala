...
class GzipFilter @Inject()(config: GzipFilterConfig) extends EssentialFilter {
      ...
      if (result.connection == HttpConnection.Close) {
        ...
        def buffer(chunks: List[Array[Byte]],
          ...
          Cont {
            ...
            case Input.El(data)
                if count <= GzipFilter.GzipHeaderLength ||
                count + data.length < config.chunkedThreshold =>
  ...
  /**
    ...
    */
  private def isNotChunked(header: ResponseHeader) =
    !header.headers.get(TRANSFER_ENCODING).exists(_ == CHUNKED)

  /**
    * We don't compress server sent events because these must be pushed immediately, and compressing buffers.
    */
  private def isNotServerSentEvents(header: ResponseHeader) =
    !header.headers.get(CONTENT_TYPE).exists(_ == MimeTypes.EVENT_STREAM)
