...
object CORSActionBuilderSpec extends CORSCommonSpec {
  ...
  def withApplication[T](conf: Map[String, _ <: Any] = Map.empty)(block: => T): T = {
    running(FakeApplication(
            withRoutes = {
          case _ =>
            CORSActionBuilder(Configuration.reference ++ Configuration.from(conf))(Results.Ok)
        }
        ))(block)
  }

  def withApplicationWithPathConfiguredAction[T](
      configPath: String, conf: Map[String, _ <: Any] = Map.empty)(block: => T): T = {
    running(FakeApplication(
            additionalConfiguration = conf,
            withRoutes = {
          case _ =>
            CORSActionBuilder(Configuration.reference ++ Configuration.from(conf), configPath)(
                Results.Ok)
        }
        ))(block)
  }
  ...
  "The CORSActionBuilder with" should {
    ...
    val restrictOriginsPathConf =
      Map("myaction.allowedOrigins" -> Seq("http://example.org", "http://localhost:9000"))
...
trait CORSCommonSpec extends PlaySpecification {
  ...
  def commonTests = {
    ...
    "forbid an empty Access-Control-Request-Method header in a preflight request" in withApplication() {
      val result =
        route(fakeRequest("OPTIONS", "/").withHeaders(ORIGIN -> "http://localhost",
    ...
    val restrictOrigins = Map(
        "play.filters.cors.allowedOrigins" -> Seq("http://example.org", "http://localhost:9000"))
