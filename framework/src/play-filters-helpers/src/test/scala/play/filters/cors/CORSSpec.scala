...
object CORSActionBuilderSpec extends CORSCommonSpec {
  ...
  def withApplication[T](conf: Map[String, _ <: Any] = Map.empty)(
    ...
    running(FakeApplication(
            withRoutes = {
          case _ =>
            CORSActionBuilder(Configuration.reference ++ Configuration.from(
                    conf))(Results.Ok)
        }
        ))(block)
  }
  ...
  "The CORSActionBuilder with" should {
    ...
    val restrictOriginsPathConf =
      Map("myaction.allowedOrigins" -> Seq("http://example.org",
                                           "http://localhost:9000"))
...
trait CORSCommonSpec extends PlaySpecification {
  ...
  def commonTests = {
    ...
    val restrictOrigins = Map("play.filters.cors.allowedOrigins" -> Seq(
            "http://example.org", "http://localhost:9000"))
