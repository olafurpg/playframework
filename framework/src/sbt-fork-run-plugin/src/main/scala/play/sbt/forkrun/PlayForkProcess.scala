...
case class PlayForkOptions(workingDirectory: File,
...
object PlayForkProcess {
  ...
  def run(workingDirectory: File,
    ...
    val errorThread = spawn {
      BasicIO.processFully(logLine(log, Level.Error))(process.getErrorStream)
    }
    def stop(): Unit = {
  ...
  def timedWaitFor(process: JProcess, millis: Long): Option[Int] =
    try {
      ...
      case _: IllegalThreadStateException =>
        Thread.sleep(100)
        if (millis > 0) timedWaitFor(process, millis - 100)
        else None
    }

  def makeOptions(jvmOptions: Seq[String],
                  classpath: Seq[File],
                  mainClass: String,
  ...
  def optionsTooLong(options: Seq[String]): Boolean =
