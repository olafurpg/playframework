...
class BoneConnectionPool @Inject()(environment: Environment)
  ...
  def create(
    ...
    val isolation = config
      .getOptionalDeprecated[String]("bonecp.isolation", "isolation")
      .map {
    ...
    val catalog = config
      .getOptionalDeprecated[String]("defaultCatalog", "bonecp.defaultCatalog")
    ...
    datasource.setAcquireIncrement(config
    ...
    datasource.setConnectionTimeoutInMs(config
          ...
          .toMillis)
    datasource.setIdleMaxAgeInSeconds(config
