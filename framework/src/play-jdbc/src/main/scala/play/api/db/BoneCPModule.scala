...
class BoneConnectionPool @Inject()(environment: Environment) extends ConnectionPool {
  ...
  def create(name: String, dbConfig: DatabaseConfig, conf: Config): DataSource = {
    ...
    val isolation = config.getOptionalDeprecated[String]("bonecp.isolation", "isolation").map {
      ...
      case unknown =>
        throw config.reportError("bonecp.isolation", s"Unknown isolation level [$unknown]")
    }
    ...
    datasource.setMaxConnectionsPerPartition(config
          .getDeprecated[Int]("bonecp.maxConnectionsPerPartition", "maxConnectionsPerPartition"))
    datasource.setMinConnectionsPerPartition(config
          .getDeprecated[Int]("bonecp.minConnectionsPerPartition", "minConnectionsPerPartition"))
    ...
    datasource.setAcquireRetryAttempts(
        config.getDeprecated[Int]("bonecp.acquireRetryAttempts", "acquireRetryAttempts"))
    datasource.setAcquireRetryDelayInMs(config
          .getDeprecated[FiniteDuration]("bonecp.acquireRetryDelay", "acquireRetryDelay")
          .toMillis)
    datasource.setConnectionTimeoutInMs(config
          .getDeprecated[FiniteDuration]("bonecp.connectionTimeout", "connectionTimeout")
          .toMillis)
    datasource.setIdleMaxAgeInSeconds(
        config.getDeprecated[FiniteDuration]("bonecp.idleMaxAge", "idleMaxAge").toSeconds)
    datasource.setMaxConnectionAgeInSeconds(config
          .getDeprecated[FiniteDuration]("bonecp.maxConnectionAge", "maxConnectionAge")
          .toSeconds)
    datasource.setDisableJMX(config.getDeprecated[Boolean]("bonecp.disableJMX", "disableJMX"))
    datasource.setStatisticsEnabled(
        config.getDeprecated[Boolean]("bonecp.statisticsEnabled", "statisticsEnabled"))
    datasource.setIdleConnectionTestPeriodInSeconds(config
          .getDeprecated[FiniteDuration](
              "bonecp.idleConnectionTestPeriod", "idleConnectionTestPeriod")
          .toSeconds)
    datasource.setDisableConnectionTracking(config
          .getDeprecated[Boolean]("bonecp.disableConnectionTracking", "disableConnectionTracking"))
    datasource.setQueryExecuteTimeLimitInMs(config
          .getDeprecated[FiniteDuration]("bonecp.queryExecuteTimeLimit", "queryExecuteTimeLimit")
          .toMillis)
    ...
    config
      .getOptionalDeprecated[String]("bonecp.initSQL", "initSQL")
      .foreach(datasource.setInitSQL)
    config
      .getOptionalDeprecated[String]("bonecp.connectionTestStatement", "connectionTestStatement")
