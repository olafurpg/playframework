...
object ScriptSpec extends Specification {
  ...
  "Conflicts" should {
    ...
    "be noticed on the most recent one" in {
      ...
      val upRest =
        Evolution(9, "DifferentDummySQLUP", "DifferentDummySQLDOWN") +: (1 to 8).reverse
    ...
    "be noticed in the middle" in {
      ...
      val downRest = (1 to 9).reverse.map(i => Evolution(i, s"DummySQLUP$i", s"DummySQLDOWN$i"))
      val upRest =
        (6 to 9).reverse.map(i => Evolution(i, s"DummySQLUP$i", s"DummySQLDOWN$i")) ++: Evolution(
    ...
    "be noticed on the first" in {
      ...
      val downRest = (1 to 9).reverse.map(i => Evolution(i, s"DummySQLUP$i", s"DummySQLDOWN$i"))
      val upRest =
        (2 to 9).reverse.map(i => Evolution(i, s"DummySQLUP$i", s"DummySQLDOWN$i")) ++: List(
