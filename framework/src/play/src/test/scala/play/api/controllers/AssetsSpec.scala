...
object AssetsSpec extends Specification {
  ...
  "Assets controller" should {
    ...
    "look up assets without percent-decoding the base path" in {
      ...
      Assets.resourceNameAt("/1%20+%202%20=%203", "x") must beSome(
          "/1%20+%202%20=%203/x")
    }
    ...
    "look up assets with percent-encoded resource paths" in {
      Assets.resourceNameAt("/x", "1%20+%202%20=%203") must beSome(
          "/x/1 + 2 = 3")
      Assets.resourceNameAt("/x", "foo%20bar.txt") must beSome(
          "/x/foo bar.txt")
    ...
    "look up assets even if the file path is a valid URI" in {
      Assets.resourceNameAt("/a", "http://localhost/x") must beSome(
          "/a/http:/localhost/x")
      Assets.resourceNameAt("/a", "//localhost/x") must beSome(
          "/a/localhost/x")
      Assets.resourceNameAt("/a", "../") must beNone
    }
    ...
    "use the unescaped path when finding the last modified date of an asset" in {
      val url =
        AssetsSpec.getClass.getClassLoader.getResource("file withspace.css")
      val assetInfo = new AssetInfo("file withspace.css", url, None, None)
      val lastModified =
        AssetInfo.dateFormat.parseDateTime(assetInfo.lastModified.get)
