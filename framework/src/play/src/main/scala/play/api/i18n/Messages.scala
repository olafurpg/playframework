...
case class Lang(language: String, country: String = "") {
  ...
  /**
    ...
    */
  lazy val toLocale = Option(country)
    .filterNot(_.isEmpty)
    .map(c => new java.util.Locale(language, c))
    .getOrElse(new java.util.Locale(language))
  ...
  /**
    ...
    */
  def satisfies(accept: Lang) =
    language.equalsIgnoreCase(accept.language) &&
    (accept match {
          case Lang(_, "") => true
  ...
  /**
    ...
    */
  lazy val code =
    language.toLowerCase(java.util.Locale.ENGLISH) + Option(country)
      .filterNot(_.isEmpty)
...
class DefaultLangs @Inject()(configuration: Configuration) extends Langs {
  ...
  val availables = {
    ...
    langs.map { lang =>
      try { Lang(lang) } catch {
        case NonFatal(e) =>
          throw configuration.reportError("play.modules.i18n.langs",
...
object Messages {
  ...
  def parse(
      ...
      messageSourceName: String): Either[PlayException.ExceptionSource, Map[String, String]] = {
  ...
  private[i18n] case class Message(
      key: String, pattern: String, source: MessageSource, sourceName: String) extends Positional

  /**
    * Message file Parser.
    */
  private[i18n] class MessagesParser(messageSource: MessageSource, messageSourceName: String)
...
class DefaultMessagesApi @Inject()(
    environment: Environment, configuration: Configuration, langs: Langs) extends MessagesApi {
  ...
  def setLang(result: Result, lang: Lang) =
    result.withCookies(Cookie(langCookieName,
                              lang.code,
                              path = Session.path,
  ...
  def clearLang(result: Result) =
    result.discardingCookies(DiscardingCookie(langCookieName,
                                              path = Session.path,
                                              domain = Session.domain,
                                              secure = langCookieSecure))
  ...
  def translate(key: String, args: Seq[Any])(implicit lang: Lang): Option[String] = {
    val langsToTry: List[Lang] = List(lang,
                                      Lang(lang.language, ""),
                                      Lang("default", ""),
                                      Lang("default.play", ""))
    val pattern: Option[String] = langsToTry.foldLeft[Option[String]](None)(
        (res, lang) => res.orElse(messages.get(lang.code).flatMap(_.get(key))))
    pattern.map(pattern =>
