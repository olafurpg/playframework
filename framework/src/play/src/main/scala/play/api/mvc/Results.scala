...
/**
  ...
  */
case class Codec(val charset: String)(
    val encode: String => Array[Byte], val decode: Array[Byte] => String)
...
trait Results {
    ...
    new Enumeratee[Array[Byte], Array[Byte]] {
      def applyOn[A](inner: Iteratee[Array[Byte], A]) = {
        ...
        val chunkedInner: Iteratee[Array[Byte], Iteratee[Array[Byte], A]] =
          ...
          Enumeratee.filterNot[Array[Byte]](_.isEmpty) ><> // Apply the chunked encoding
          formatChunks ><> // Don't feed EOF into the iteratee - so we can feed the last chunk ourselves later
          Enumeratee.passAlong &> // And apply the inner iteratee
          inner
  ...
  def dechunkWithTrailers: Enumeratee[
    ...
    dechunk0 ><> Enumeratee.mapFlatten[ChunkOrTrailer][ChunkOrTrailer] {
      ...
      case r@Right(_) =>
        Enumerator[ChunkOrTrailer](r) >>> Enumerator.eof[ChunkOrTrailer]
    }
  }
  ...
  private def dechunk0: Enumeratee[
    ...
    def readLastChunk: Iteratee[Array[Byte], List[(String, String)]] =
      for {
    ...
    val chunkParser: Iteratee[
      ...
      chunk <- if (size > 0) readChunk(size).map(Left.apply)
              else readLastChunk.map(Right.apply)
    } yield chunk
  ...
  val NoContent = Result(header = ResponseHeader(NO_CONTENT),
                         body = Enumerator.empty,
                         connection = HttpConnection.KeepAlive)

  /** Generates a ‘205 RESET_CONTENT’ result. */
  val ResetContent = Result(header = ResponseHeader(RESET_CONTENT),
                            body = Enumerator.empty,
                            connection = HttpConnection.KeepAlive)
  ...
  val NotModified = Result(header = ResponseHeader(NOT_MODIFIED),
                           body = Enumerator.empty,
                           connection = HttpConnection.KeepAlive)
