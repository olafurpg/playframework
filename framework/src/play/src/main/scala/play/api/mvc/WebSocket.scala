...
object WebSocket {
  ...
  object FrameFormatter {
    ...
    /**
      ...
      */
    implicit val mixedFrame: FrameFormatter[Either[String, Array[Byte]]] =
    ...
    implicit val jsonFrame: FrameFormatter[JsValue] =
      stringFrame.transform(Json.stringify, Json.parse)

    /**
      * Json WebSocket frames, parsed into/formatted from objects of type A.
      */
    def jsonFrame[A : Format]: FrameFormatter[A] =
      jsonFrame.transform[A](
          out => Json.toJson(out),
          in =>
            Json
              .fromJson[A](in)
