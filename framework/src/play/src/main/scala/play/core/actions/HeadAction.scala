...
/**
  ...
  */
class HeadAction(action: EssentialAction)
    extends EssentialAction with DefaultWriteables with HttpProtocol {
  ...
  def apply(requestHeader: RequestHeader): Iteratee[Array[Byte], Result] = {
    ...
    def createHeadResult(result: Result): Future[Result] = result match {
      ...
      case UsesTransferEncoding() | HasContentLength() =>
        val newResult = Result(
            result.header, Enumerator(Array.emptyByteArray), result.connection)
    ...
    bodyIterator.mapM(result => createHeadResult(result))
  }
  ...
  /**
    ...
    * @return
    */
  def singleChunkIteratee(
      result: Result, httpVersion: String): Iteratee[Array[Byte], Result] = {
    lazy val resultWithEmptyBody = Result(
        result.header, Enumerator(Array.emptyByteArray), result.connection)

    def takeUpToOneChunk(chunk: Option[Array[Byte]]
        ): Iteratee[Array[Byte], Either[Array[Byte], Option[Array[Byte]]]] =
      Cont {
...
object UsesTransferEncoding {
  def unapply(result: Result): Boolean =
    result.header.headers.contains(TRANSFER_ENCODING)
}
...
object HasContentLength {
  def unapply(result: Result): Boolean =
    result.header.headers.contains(CONTENT_LENGTH)
}
