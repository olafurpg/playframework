...
private class TaggingInvoker[A](
  ...
  def call(call: => A): Handler = {
    ...
    handler match {
      ...
      case ws@WebSocket(f) =>
        WebSocket[ws.FramesIn, ws.FramesOut](
            rh => ws.f(taggedRequest(rh, cachedHandlerTags)))(
...
object HandlerInvokerFactory {
  ...
  implicit def javaBytesWebSocket: HandlerInvokerFactory[JWebSocket[
          Array[Byte]]] =
    new JavaWebSocketInvokerFactory[JWebSocket[Array[Byte]], Array[Byte]] {
      def webSocketCall(call: => JWebSocket[Array[Byte]]) =
  ...
  implicit def javaStringWebSocket: HandlerInvokerFactory[JWebSocket[String]] =
    new JavaWebSocketInvokerFactory[JWebSocket[String], String] {
      def webSocketCall(call: => JWebSocket[String]) =
  ...
  implicit def javaJsonPromiseWebSocket: HandlerInvokerFactory[JPromise[
          JWebSocket[JsonNode]]] =
