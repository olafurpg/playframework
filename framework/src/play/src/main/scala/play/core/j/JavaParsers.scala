...
object JavaParsers extends BodyParsers {
  ...
  def default_(maxLength: Long): BodyParser[RequestBody] =
    anyContent(parse.default(Some(maxLength).filter(_ >= 0)))
  ...
  def tolerantJson(maxLength: Long): BodyParser[RequestBody] =
    parse
      .tolerantJson(orDefault(maxLength))
      .map { json =>
        DefaultRequestBody(json = Some(json))
      }(trampoline)
  ...
  def tolerantXml(maxLength: Long): BodyParser[RequestBody] =
    parse
      .tolerantXml(orDefault(maxLength))
      .map { xml =>
        DefaultRequestBody(xml = Some(xml))
      }(trampoline)
  ...
  def tolerantText(maxLength: Long): BodyParser[RequestBody] =
    parse
      .tolerantText(orDefault(maxLength))
      .map { text =>
        DefaultRequestBody(text = Some(text))
      }(trampoline)
  ...
  def formUrlEncoded(maxLength: Long): BodyParser[RequestBody] =
    parse
      .urlFormEncoded(orDefault(maxLength))
      .map { urlFormEncoded =>
  ...
  def raw(maxLength: Long): BodyParser[RequestBody] =
    parse
      .raw(parse.DefaultMaxTextLength, orDefault(maxLength))
      .map { raw =>
        DefaultRequestBody(raw = Some(raw))
      }(trampoline)

  def empty(): BodyParser[RequestBody] =
    parse.empty.map { (_: Unit) =>
      new RequestBody()
    }(trampoline)

  private def orDefault(maxLength: Long) =
    if (maxLength < 0) parse.DefaultMaxTextLength else maxLength.toInt
}
