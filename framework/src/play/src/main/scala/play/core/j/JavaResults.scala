...
object JavaResults
  ...
  def chunked[A](onConnected: play.libs.F.Callback[Channel[A]],
                 onDisconnected: play.libs.F.Callback0): Enumerator[A] = {
  ...
  def sendFile(status: play.api.mvc.Results.Status,
               file: java.io.File,
               inline: Boolean,
               filename: String) = status.sendFile(file, inline, _ => filename)
  def sendPath(status: play.api.mvc.Results.Status,
...
object JavaResultExtractor {
  ...
  def getCookies(result: JResult): JCookies = new JCookies {
