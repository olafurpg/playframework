...
object Multipart {
  ...
  private def parseParts(dataPartLimit: Int,
    ...
    parsePart(dataPartLimit, filePartHandler).flatMap {
      ...
      case Some(MaxDataPartSizeExceeded(_)) =>
        Done(Left(Results.EntityTooLarge))
  ...
  private def handleDataPart(maxLength: Int): PartHandler[Part] = {
    case headers@PartInfoMatcher(partName) if !FileInfoMatcher.unapply(headers).isDefined =>
      Traversable
        ...
        .transform(Iteratee
              .consume[Array[Byte]]()
              .map(bytes => DataPart(partName, new String(bytes, "utf-8"))))
        .flatMap { data =>
          Cont({
            case Input.El(_) =>
              Done(MaxDataPartSizeExceeded(partName), Input.Empty)
