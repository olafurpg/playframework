...
private[play] object FPromiseHelper {
  ...
  def delayed[A](callable: Callable[A],
                 duration: Long,
                 unit: TimeUnit,
                 ec: ExecutionContext): F.Promise[A] =
    delayedWith(callable.call(), duration, unit, ec)

  def delayed[A](function: F.Function0[A],
                 duration: Long,
                 unit: TimeUnit,
                 ec: ExecutionContext): F.Promise[A] =
  ...
  def get[A](promise: F.Promise[A], timeout: Long, unit: TimeUnit): A = {
    try {
      ...
      case ex: TimeoutException =>
        throw new F.PromiseTimeoutException(ex.getMessage, ex)
    }
  }
  ...
  def timeout[A](delay: Long, unit: TimeUnit): F.Promise[A] =
    timeoutWith(Failure(new F.PromiseTimeoutException("Timeout in promise")),
                delay,
                unit)
  ...
  def onFailure[A](promise: F.Promise[A],
                   action: F.Callback[Throwable],
                   ec: ExecutionContext) {
    promise.wrapped().onFailure { case t => action.invoke(t) }(ec.prepare())
  }

  def transform[A, B, T >: A](promise: F.Promise[A],
                              s: F.Function[T, B],
  ...
  def completeWith[T](promise: Promise[T],
                      other: Future[T],
                      ec: ExecutionContext): Future[Void] = {
    val p = Promise[Unit]
    other.onComplete { x =>
  ...
  def tryCompleteWith[T](promise: Promise[T],
                         other: Future[T],
