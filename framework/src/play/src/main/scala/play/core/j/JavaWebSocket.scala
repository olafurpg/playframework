...
object JavaWebSocket extends JavaHelpers {
  ...
  def webSocketWrapper[A](retrieveWebSocket: => Future[JWebSocket[A]])(
      implicit frameFormatter: WebSocket.FrameFormatter[A],
      mt: ClassTag[A]): WebSocket[A, A] = WebSocket[A, A] { request =>
        ...
        Right((in, out) =>
            ...
            if (jws.isActor) {
              ...
              WebSocketsExtension(Akka.system).actor ! WebSocketsActor.Connect(
                  request.id, in, out, actorRef => jws.actorProps(actorRef))
            } else {
              ...
              in |>> {
                Iteratee
                  .foreach[A](
                      msg => socketIn.callbacks.asScala.foreach(_.invoke(msg)))
                  .map { _ =>
  ...
  def ofBytes(retrieveWebSocket: => JWebSocket[Array[Byte]]
      ): WebSocket[Array[Byte], Array[Byte]] =
    webSocketWrapper[Array[Byte]](Future.successful(retrieveWebSocket))

  def promiseOfBytes(retrieveWebSocket: => JPromise[JWebSocket[Array[Byte]]]
      ): WebSocket[Array[Byte], Array[Byte]] =
  ...
  def ofString(
      retrieveWebSocket: => JWebSocket[String]): WebSocket[String, String] =
    webSocketWrapper[String](Future.successful(retrieveWebSocket))

  def promiseOfString(retrieveWebSocket: => JPromise[JWebSocket[String]]
  ...
  def ofJson(retrieveWebSocket: => JWebSocket[JsonNode]
      ): WebSocket[JsonNode, JsonNode] =
    webSocketWrapper[JsonNode](Future.successful(retrieveWebSocket))

  def promiseOfJson(retrieveWebSocket: => JPromise[JWebSocket[JsonNode]]
      ): WebSocket[JsonNode, JsonNode] =
