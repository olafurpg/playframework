...
object ApplicationBuild extends Build {
  ...
  FileFilter) = enabledExternalPlayModules match {
    ...
    case Some("all") =>
      (externalPlayModules.toSeq.flatMap(_._2),
    ...
    case Some(explicit) =>
      ...
      (enabled.flatMap(e => externalPlayModules.get(e).getOrElse(Nil)),
       enabled.foldLeft[FileFilter](new ExactFilter("code")) { (filter, e) =>
         filter || new ExactFilter("code-" + e)
       })
  }
  ...
  lazy val main = Project("Play-Documentation", file("."))
    ...
    .settings(
        ...
        (baseDirectory.value / "manual" / "working" / "scalaGuide" ** codeFilter).get,
        unmanagedSourceDirectories in Test ++=
        (baseDirectory.value / "manual" / "detailedTopics" ** codeFilter).get,
