...
// #context
  ...
  class HowsMySSLSpec extends PlaySpecification {
    ...
    def createClient(rawConfig: play.api.Configuration): WSClient = {
      ...
      val parser = new WSConfigParser(
          rawConfig, new Environment(new File("."), classLoader, Mode.Test))
    ...
    "WS" should {
      ...
      "verify common behavior" in {
       ...
       """.stripMargin
        ...
        val configWithPem = rawConfig.withValue(
            "geotrust.pem", ConfigValueFactory.fromAnyRef(geoTrustPem))
        val configWithSystemProperties = ConfigFactory.load(configWithPem)
        val playConfiguration =
          play.api.Configuration(configWithSystemProperties)

        val client = createClient(playConfiguration)
        val response = await(
            client.url("https://www.howsmyssl.com/a/check").get())(5.seconds)
