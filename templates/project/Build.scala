...
object Templates {
  ...
  val templateSettings: Seq[Setting[_]] =
    s3Settings ++ Seq(
        templates := Nil,
        ...
        syncTemplates := {
          ...
          val mappings = templates.flatMap { template =>
            (template.***.filter(!_.isDirectory) x relativeTo(template))
              .map(f => (f._1, templateDir / template.getName / f._2))
          }
        ...
        prepareTemplates := {
          ...
          val mappings = templateDirs.flatMap { template =>
            (template.***.filter(fileFilter(_)) x relativeTo(template))
              .map(f => (f._1, outDir / template.getName / f._2))
          }
